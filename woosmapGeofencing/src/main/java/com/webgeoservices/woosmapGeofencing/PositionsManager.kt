...
class PositionsManager(val context: Context, private val db:WoosmapDb) {
    ...
    private fun visitsDetectionAlgo(lastVisit: Visit, location:Location) {
        ...
        //not visit in progress
            ...
            val previousMovingPosition = this.db.movingPositionsDao.lastMovingPosition ?: this.createMovingPositionFromLocation(location)
            ...
            if (distance >= 25.0){
                ...
                if (olderPosition != null){
                    ...
                    if (distanceVisit <= 25.0){ // ON EST A MOINS DE 25m de l'avant dernier point donc vraiment dans une visite
                        val visit = Visit()
                    ...
                    else { //On est juste en train de faire une "petite pause"
                        this.createMovingPositionFromLocation(location)
                    }
                }
            }
    ...
    private fun addPositionFromLocation(location: Location) {
        ...
        if (distance >= WoosmapSettings.currentLocationDistanceFilter){
            this.createMovingPositionFromLocation(location)
            Log.d(WoosmapVisitsTag, "createMovingPositionFromLocation")
        }
    }
